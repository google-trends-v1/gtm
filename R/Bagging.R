Bagging<-function(data=NULL, boot=NULL, model="ar", w_size=NULL, sim="fixed", l=12L, endcorr=TRUE, norm=TRUE, n.sim=NROW(data), window="recursive", y.index=1, ...){
  if(is.null(boot)){
  ...
  results<-tsboot(tseries=data, statistic=maeforecast.simplified, R=boot,
                  ...
                  endcorr=endcorr, y.index=y.index, ...)$t
  results<-t(results)
